...
public class Main {
    public static void main(String... args) {
        ...
        */
        ...
        System.out.println("\nGenerating Squares: ");
        final List<GridSquare> squareList = new ArrayList();

        for (int scale = 1; scale <= gridSize; scale++) {
            //System.out.println("\tScale: " + scale);
            for (int x = 0; x < (gridSize - (1 - scale)); x++) {
                for (int y = 0; y < (gridSize - (1 - scale)); y++) {

                    //Lazy ignore anything outside the bounds
                    try {
                        //Build squares
                        GridSquare square = new GridSquare(x, y, scale);
                    ...
                    {
                        if(!e.getMessage().contains("grid bounds")) {
                            e.printStackTrace();
                        }
                    }
                }
